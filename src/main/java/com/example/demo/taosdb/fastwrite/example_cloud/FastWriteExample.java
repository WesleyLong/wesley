...
public class FastWriteExample {
    ...
    public static void stopAll() {
        ...
        readTasks.forEach(ReadTask::stop);
        writeTasks.forEach(WriteTask::stop);
    ...
    public static void main(String[] args) throws InterruptedException, SQLException {
        ...
        //云端有限制 一条sql最长不能超过67777左右个字符，所以要限制每次插入的条数maxBatchSize
        //总感觉跟这个数很接近的一个数很熟悉
        int maxBatchSize = args.length > 3 ? Integer.parseInt(args[3]) : 1000;
...
//                .prepareDatabase();//云服务器不能删库建库，必须先在云端的dashboard初始化库和超级表，不知道是不是免费账户的关系

        // Create task queues, writing tasks and start writing threads.
