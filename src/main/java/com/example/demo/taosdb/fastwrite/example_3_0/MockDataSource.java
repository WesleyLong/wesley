...
class MockDataSource implements Iterator<String> {
    ...
    // mock values
    String[] location = {"LosAngeles", "SanDiego", "SanJose", "Campbell", "SanFrancisco"};
    ...
    public String next() {
        ...
        int index = currentRow % 5;
        return tbNamePrefix + "_" + currentTbId + "," + ts + ',' + // ts
                (current[index] + Math.random()) + ',' + // current
                voltage[index] + ',' + // voltage
                (phase[index] + Math.random()) + ',' + // phase
                location[(int) (Math.random() * 5)] + ',' + // location
                groupId;
    }
}
